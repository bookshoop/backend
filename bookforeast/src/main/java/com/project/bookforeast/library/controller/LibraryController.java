...
public class LibraryController {
    ...
    public ResponseEntity<Void> insLibrary(@RequestBody @Valid SimpleLibraryDTO simpleLibraryDTO) {
        libraryService.insLibrary(simpleLibraryDTO);
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "401",
						 ...
						 content = @Content(schema = @Schema(example = "{\"code\" : \"401 UNAUTHORIZED\", \"message\" : \"message\"}"))),
			@ApiResponse(responseCode = "200",
						 description = "서재정보 가져오기 성공",
                         content = @Content(schema = @Schema(implementation = LibraryDTO.class))
                         ),
	})
    public ResponseEntity<LibraryDTO> getLibraryInfos() {
        LibraryDTO libraryDTO = libraryService.getLibraryInfos();
